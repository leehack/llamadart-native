...
class ReleaseVersionPolicyTest(unittest.TestCase):
    ...
    def test_stable_wrapper_rebuild_is_monotonic(self) -> None:
        upstream, rebuild = validate_pair("v0.2.0", "v0.2.1-llamadart.2")
        self.assertEqual(wrapper_tag_for(upstream), "v0.2.1-llamadart.1")
        self.assertTrue(rebuild.github_prerelease)
        validate_history(rebuild, ["v0.2.0", "v0.2.1-llamadart.1"])
        validate_history(
            parse_native_tag("v0.2.1-llamadart.10"),
            ["v0.2.1-llamadart.9"],
        )
    ...
    def test_rollback_and_collision_are_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(PolicyError, "rollback"):
            validate_history(
                parse_native_tag("v0.2.1-llamadart.1"),
                ["v0.2.1-llamadart.2"],
            )
        with self.assertRaisesRegex(PolicyError, "rollback"):
            validate_history(
